...
# before Creating NAT gate way , we should create Elastic IP(provides same IP)
...
# Routes [Internet connection for Public Route Table]
...
# Privat Route Table is not allowed by public with internet access ; so we use NAT 
...
# Associating Route_Tables with Subnets
